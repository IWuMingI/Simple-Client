package cn.wuming.simple.clickgui;
...
public class ClickGUI extends GuiScreen {
    ...
    private List<cn.wuming.simple.clickgui.Panel> panels = new ArrayList<>();

    public ClickGUI() {
        int x = 202;

        for(final ModuleCategory moduleCategory : ModuleCategory.values()) {
            final cn.wuming.simple.clickgui.Panel panel = new cn.wuming.simple.clickgui.Panel(moduleCategory.getName(), x, 50, 100);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        for(final cn.wuming.simple.clickgui.Panel panel : panels) {
            // Draw panel
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for(int index = panels.size() - 1; index >= 0; index--) {
            final cn.wuming.simple.clickgui.Panel panel = panels.get(index);
